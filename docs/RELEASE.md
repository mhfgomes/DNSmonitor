...
Dispatching this workflow publishes both the image and the GitHub release. Create `mhfgomes/dnsmonitor` in Docker Hub with the intended public/private visibility before dispatching. After publishing the GitHub release, the workflow updates `mhfgomes/dnsmonitor:latest` to the same multi-platform image digest. `latest` follows the last successfully published release; version tags remain available for pinned deployments. To update an existing installation using `latest`, pull the image and recreate the containers. The manual **Promote existing image to latest** workflow can also repoint it to an already published release without rebuilding. A failed GitHub release step can leave a successfully published image, so inspect the registry before retrying. Treat version tags as immutable; choose a new patch version for changed source.
